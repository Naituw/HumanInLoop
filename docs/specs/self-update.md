...
| D5 | 生效时机（关键，无开关） | **无开关**。更新是用户主动触发（弹窗浮层、设置或菜单栏内点击）。触发 = 把新二进制落盘，并即时用 Hello 指纹握手通知已运行 daemon：空闲时立即换新，有在途时进入 drain、待全部答完后生效，**绝不退出/打断当前作答**；15 秒指纹轮询只作外部更新与漏通知兜底。界面给出当前等待阶段与原因。 |
